# 文件监视和同步工具

## 🎯 功能

自动监视 `static/` 目录的文件变化，当检测到图片文件的增删改时：

1. 自动判断是哪个产品
2. 自动生成该产品的文件列表
3. 自动同步到 server 端

## 📦 安装依赖

```bash
pip3 install watchdog
```

## 🚀 使用方法

### 方式 1: 使用启动脚本（推荐）

```bash
./start-watch.sh
```

### 方式 2: 直接运行 Python 脚本

```bash
python3 tools/sync-static-files/watch-and-sync.py
```

## 📋 工作流程

```
1. 监视 static/ 目录
   ↓
2. 检测到图片文件变化
   ↓
3. 判断产品 (如 headshot-ai)
   ↓
4. 生成文件列表
   tools/filelist-generator/headshot-ai/files.txt
   ↓
5. 同步到 server 端
   /path/to/aipso-server/static/headshot-ai/files.txt
```

## 🎬 示例

### 启动监视

```bash
$ ./start-watch.sh

============================================================
🔍 文件监视和同步工具
============================================================

📁 监视目录: /path/to/aipso-static/static
📝 生成器: /path/to/tools/filelist-generator/generate-filelist.py
🎯 同步目标: /path/to/aipso-server/static

💡 提示:
   • 当 static/ 目录下的图片文件发生变化时
   • 自动生成文件列表
   • 自动同步到 server 端
   • 使用 Ctrl+C 停止监视

============================================================

✅ 监视已启动，等待文件变化...
```

### 添加文件时

```bash
# 在另一个终端添加文件
$ cp new-image.webp static/headshot-ai/images/home/City/

# 监视工具自动输出：
============================================================
📁 检测到变化: headshot-ai
   事件类型: created
   文件: new-image.webp
============================================================

🔄 步骤 1: 生成文件列表...
✅ 文件列表生成成功
   📊 总计 1098 个文件
   💾 文件大小: 54.38 KB

🔄 步骤 2: 同步到 server 端...
✅ 同步成功
   源: tools/filelist-generator/headshot-ai/files.txt
   目标: /path/to/aipso-server/static/headshot-ai/files.txt
   大小: 54.38 KB

============================================================
✅ 处理完成: headshot-ai
============================================================
```

### 停止监视

按 `Ctrl+C` 停止：

```bash
^C
🛑 停止监视...
👋 已退出
```

## ⚙️ 配置

编辑 `tools/sync-static-files/watch-and-sync.py` 中的配置：

```python
# Server static 目录路径
SERVER_STORE_DIR = Path('/Users/luyunfei/Desktop/.../aipso-server/static')

# 防抖动时间（秒）
DEBOUNCE_SECONDS = 2

# 支持的图片格式
IMAGE_EXTENSIONS = {'.webp', '.jpg', '.jpeg', '.png', '.gif'}
```

## 🔍 监视的事件

工具会监视以下文件系统事件：

- `created` - 文件创建
- `modified` - 文件修改
- `deleted` - 文件删除
- `moved` - 文件移动

## 🚫 忽略的文件

- 非图片文件（根据扩展名判断）
- 隐藏文件（以 `.` 开头）
- 目录变化

## ⚡ 防抖动机制

为避免短时间内重复触发，工具内置了防抖动机制：

- 同一产品在 2 秒内只会触发一次处理
- 避免批量操作时产生大量重复处理

## 🔧 故障排查

### 问题 1: 缺少 watchdog 依赖

```bash
❌ 缺少依赖: watchdog

请安装依赖:
  pip install watchdog
```

**解决方法**：
```bash
pip3 install watchdog
```

### 问题 2: Server static 目录不存在

```bash
❌ 路径检查失败:
   • Server static 目录不存在: /path/to/aipso-server/static
```

**解决方法**：
1. 检查 server 项目路径是否正确
2. 修改 `tools/sync-static-files/watch-and-sync.py` 中的 `SERVER_STORE_DIR` 配置

### 问题 3: 权限问题

```bash
❌ 同步失败: [Errno 13] Permission denied
```

**解决方法**：
```bash
# 检查目标目录权限
ls -ld /path/to/aipso-server/static/headshot-ai/

# 如果需要，修改权限
chmod 755 /path/to/aipso-server/static/headshot-ai/
```

## 🎯 使用场景

### 场景 1: 开发环境

```bash
# 终端 1: 启动静态文件服务器
python3 dev_server.py

# 终端 2: 启动文件监视
./start-watch.sh

# 终端 3: 启动 server 端
cd ../aipso-server
npm run dev

# 现在添加图片会自动同步到 server 端
cp new-image.webp static/headshot-ai/images/home/City/
```

### 场景 2: 批量导入图片

```bash
# 启动监视
./start-watch.sh

# 批量复制图片（会自动触发一次处理）
cp -r new-images/* static/headshot-ai/images/home/City/
```

### 场景 3: 测试环境

```bash
# 在测试环境持续运行监视工具
nohup python3 tools/sync-static-files/watch-and-sync.py > watch.log 2>&1 &

# 查看日志
tail -f watch.log
```

## 📊 性能考虑

- **CPU 占用**：监视工具本身占用很少的 CPU
- **内存占用**：约 20-30 MB
- **响应时间**：检测到变化后 2 秒内开始处理
- **处理时间**：生成文件列表约 1-2 秒（取决于文件数量）

## 🔄 与手动方式对比

| 操作 | 手动方式 | 自动监视 |
|------|---------|---------|
| 添加图片 | 1. 复制图片<br>2. 运行生成脚本<br>3. 复制文件列表 | 1. 复制图片<br>（自动完成其他步骤） |
| 响应时间 | 需要手动操作 | 2 秒内自动处理 |
| 出错风险 | 可能忘记更新 | 自动保证同步 |
| 适用场景 | 偶尔更新 | 频繁更新 |

## 💡 最佳实践

1. **开发时启动**：在开发环境中持续运行监视工具
2. **批量操作**：批量添加文件时，防抖动机制会自动合并处理
3. **日志记录**：生产环境建议重定向输出到日志文件
4. **定期检查**：定期检查 server 端的文件列表是否正确同步

## 🚀 进阶用法

### 后台运行

```bash
# 后台运行
nohup ./start-watch.sh > watch.log 2>&1 &

# 查看进程
ps aux | grep watch-and-sync

# 停止进程
kill <PID>
```

### 使用 systemd（Linux）

创建服务文件 `/etc/systemd/system/watch-and-sync.service`：

```ini
[Unit]
Description=File Watch and Sync Service
After=network.target

[Service]
Type=simple
User=your-user
WorkingDirectory=/path/to/aipso-static
ExecStart=/usr/bin/python3 /path/to/aipso-static/tools/sync-static-files/watch-and-sync.py
Restart=always

[Install]
WantedBy=multi-user.target
```

启动服务：
```bash
sudo systemctl enable watch-and-sync
sudo systemctl start watch-and-sync
sudo systemctl status watch-and-sync
```

## 📝 总结

文件监视和同步工具提供了：

- ✅ 自动检测文件变化
- ✅ 自动生成文件列表
- ✅ 自动同步到 server 端
- ✅ 防抖动机制
- ✅ 清晰的日志输出
- ✅ 简单易用

让开发流程更加流畅，无需手动维护文件列表！🎉
