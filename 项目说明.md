# Store 静态资源服务文档

## 📋 概述

aip.so 是一个专业的 AI 摄影平台，为用户提供 Business Headshot AI、Group Photo AI、Fashion Shot AI 等不同细分领域的产品，满足用户在该领域的专业摄影需求。

本项目是从 aip.so 的后端服务的 `static` 目录分离出来的，作为独立的静态资源服务部署，提供给开发环境，以及将资源文件从这里上传到 Cloudflare，提供给生产环境使用。


## 📁 目录结构

### 完整结构

```
static/
├── {product-slug}/              # 产品目录（如 business-headshot-ai, group-photo-ai）
│   ├── images/                  # 图片资源
│   │   ├── demo/               # 演示图片
│   │   │   ├── home/           # 首页展示图片
│   │   │   │   ├── {category}/
│   │   │   │   │   └── *.webp
│   │   │   ├── faces/          # 演示人脸
│   │   │   │   ├── {gender}/  # Female, Male
│   │   │   │   │   ├── {ethnicity}/  # White, Black, Asian, etc.
│   │   │   │   │   │   └── *.webp
│   │   │   └── options/        # 选项图片
│   │   │       ├── backdrops/  # 背景图片
│   │   │       │   ├── {category}/
│   │   │       │   │   ├── {image-name}/
│   │   │       │   │   │   ├── blur-0.webp   # 原始图片
│   │   │       │   │   │   ├── blur-5.webp   # 轻度模糊
│   │   │       │   │   │   ├── blur-10.webp  # 中度模糊
│   │   │       │   │   │   └── blur-15.webp  # 重度模糊
│   │   │       ├── poses/      # 姿势图片
│   │   │       ├── outfits/    # 服装图片
│   │   │       └── hairstyles/ # 发型图片
│   │   └── {width}/            # 不同尺寸的图片（如 demo, 400, 800）
│   │       ├── home/
│   │       └── faces/
└── README.md                    # 本文档
```

### 示例：business-headshot-ai 产品

```
static/business-headshot-ai/
├── images/
│   ├── demo/
│   │   ├── home/
│   │   │   ├── Business/
│   │   │   │   ├── business-1.webp
│   │   │   │   └── business-2.webp
│   │   │   └── Casual/
│   │   │       └── casual-1.webp
│   │   ├── faces/
│   │   │   ├── Female/
│   │   │   │   ├── White/
│   │   │   │   │   ├── face-1.webp
│   │   │   │   │   └── face-2.webp
│   │   │   │   └── Asian/
│   │   │   │       └── face-1.webp
│   │   │   └── Male/
│   │   │       └── White/
│   │   │           └── face-1.webp
│   │   └── options/
│   │       ├── backdrops/
│   │       │   ├── 1@Studio/
│   │       │   │   ├── Dark-Gradients/
│   │       │   │   │   ├── blur-0.webp
│   │       │   │   │   ├── blur-5.webp
│   │       │   │   │   ├── blur-10.webp
│   │       │   │   │   └── blur-15.webp
│   │       │   │   └── Light-Textures/
│   │       │   │       └── blur-*.webp
│   │       │   └── 2@Outdoor/
│   │       │       └── Beach/
│   │       │           └── blur-*.webp
│   │       ├── poses/
│   │       ├── outfits/
│   │       └── hairstyles/
│   └── 400/                     # 400px宽度的缩略图
│       ├── home/
│       └── faces/

```

## 客户端如何访问

```javascript
// 配置静态资源域名
const STATIC_DOMAIN = 'https://static.aip.so';
const PRODUCT = 'business-headshot-ai';

// 直接访问图片
const imageUrl = `${STATIC_DOMAIN}/${PRODUCT}/images/options/backdrops/1@Studio/Dark-Gradients/blur-0.webp`;

<img src={imageUrl} alt="Backdrop" />
```

## 🚀 部署方案

### 生产环境：Cloudflare Pages（推荐）

将 static 静态资源部署到 Cloudflare Pages：

- ✅ 无限带宽，零成本
- ✅ 全球 300+ CDN 节点
- ✅ 自动 HTTPS
- ✅ Git 自动部署

详见：[Store Cloudflare 部署指南](./__docs/Static_Cloudflare部署指南.md)

### 开发环境：Python 静态服务器

最简单的开发方式，无需配置 Nginx：

```bash
# 启动服务器
python3 dev_server.py

# 访问测试页面
open http://localhost:8080/test.html

# 或直接访问图片
open http://localhost:8080/business-headshot-ai/images/home/Studio/Light/15.webp
```

**特性**：
- ✅ 一键启动，无需配置
- ✅ 支持多产品架构（路径或子域名）
- ✅ 自动 CORS 支持
- ✅ 目录浏览功能
- ✅ 清晰的日志输出

详见：[开发服务器文档](./DEV_SERVER.md)

### 2. 测试环境：Nginx静态服务

```nginx
server {
    listen 80;
    server_name static.aip.so;

    # 静态资源根目录
    root /var/www/static;

    # 启用gzip压缩
    gzip on;
    gzip_types image/webp image/jpeg image/png;

    # 缓存配置
    location ~* \.(webp|jpg|jpeg|png|gif)$ {
        expires 30d;
        add_header Cache-Control "public, immutable";
    }

    # CORS配置（如果需要跨域访问）
    add_header Access-Control-Allow-Origin "*";
    add_header Access-Control-Allow-Methods "GET, OPTIONS";

    # 产品图片
    location ~ ^/([^/]+)/images/ {
        try_files $uri $uri/ =404;
    }

    # 用户上传文件（需要认证）
    location ~ ^/([^/]+)/upload/ {
        # 添加认证逻辑
        auth_request /auth;
        try_files $uri $uri/ =404;
    }
}
```

### 3. 生产环境：CDN + 对象存储

使用 Cloudflare



## 📝 文件命名规范

### 图片文件

- **格式**：优先使用 `.webp`，其次 `.jpg`、`.png`
- **命名**：小写字母、数字、连字符，如 `dark-gradient.webp`
- **避免**：空格、特殊字符、中文

### 目录命名

- **分类目录**：使用 `@` 前缀表示排序，如 `1@Studio`、`2@Outdoor`
- **子分类**：使用连字符，如 `Dark-Gradients`、`Light-Textures`

### 模糊图片

- **命名格式**：`blur-{level}.webp`
- **级别**：`0`（原始）、`5`（轻度）、`10`（中度）、`15`（重度）
- **位置**：在图片名称的子文件夹中


## 🔧 维护指南

### 添加新产品

1. 创建产品目录：
```bash
mkdir -p static/new-product-ai/images/{home,faces,options}
```

2. 添加演示图片：
```bash
# 首页图片
static/new-product-ai/images/home/Category1/image1.webp

# 人脸图片
static/new-product-ai/images/demo-faces/Female/White/face1.webp

# 选项图片
static/new-product-ai/images/options/backdrops/1@Studio/...
```

3. 在后端配置中启用产品（`server/settings.py`）

### 添加新背景图片

使用提供的工具生成模糊版本：

```bash
# 1. 将原始图片放入input目录
mkdir -p tools/create-backdrops-blur-image/input/1@Studio
cp original-image.webp tools/create-backdrops-blur-image/input/1@Studio/

# 2. 运行工具
cd tools/create-backdrops-blur-image
python main.py

# 3. 复制生成的文件到store
cp -r input/1@Studio/* ../../static/business-headshot-ai/images/options/backdrops/1@Studio/

```

详见：[背景图片模糊版本生成工具](../../tools/create-backdrops-blur-image/README.md)

### 更新图片

1. **替换图片**：直接覆盖原文件
2. **清除缓存**：
   - CDN：在CDN控制台刷新缓存
   - Nginx：`nginx -s reload`
   - 浏览器：添加版本号 `?v=2`

### 删除图片

1. 删除文件
2. 清除CDN缓存
3. 检查是否有API引用该图片

## 📊 性能优化

### 图片优化

```bash
# 使用cwebp压缩图片
cwebp -q 80 input.jpg -o output.webp

# 批量转换
for img in *.jpg; do
    cwebp -q 80 "$img" -o "${img%.jpg}.webp"
done
```

### 生成多尺寸

```bash
# 使用ImageMagick生成缩略图
convert input.webp -resize 400x400 output-400.webp
convert input.webp -resize 800x800 output-800.webp
```

### 缓存策略

| 资源类型 | 缓存时间 | 说明 |
|---------|---------|------|
| 演示图片 | 30天 | 很少变更 |
| 用户上传 | 7天 | 可能更新 |
| 模糊图片 | 90天 | 几乎不变 |

## 🔐 安全考虑

- ✅ 公开访问
- ✅ 防盗链
- ✅ 限制请求频率


## 📈 监控和日志

### 监控指标

- 请求量（QPS）
- 响应时间
- 缓存命中率
- 带宽使用
- 错误率（4xx, 5xx）

## 🗂️ 文件列表工具

本项目提供了文件列表生成和解析工具，用于快速管理和查询静态资源。

### 快速开始

```bash
# 生成文件列表
./generate-filelist.sh business-headshot-ai
```

### 自动监视和同步

启动文件监视工具，自动检测 static/ 目录的变化并同步到 server 端：

```bash
# 安装依赖
pip3 install watchdog

# 启动监视
./start-watch.sh
```

详见：[文件监视和同步工具](./tools/sync-static-files/README.md)

### 生成的文件

```
tools/filelist-generator/business-headshot-ai/files.txt
```

文件格式：每行一个文件路径，按字典序排序
```
images/home/City/city-1.webp
images/home/City/city-2.webp
images/options/backdrops/1@Studio/Dark-Gradients/blur-0.webp
```

### Server 端集成

```python
from tools.filelist_parser import FileListParser

# 初始化解析器
parser = FileListParser('tools/filelist-generator/business-headshot-ai/files.txt')

# 获取分页数据
page_data = parser.get_page(page=1, page_size=20)

# 按分类获取
home_files = parser.get_files_by_category('home')

# 搜索文件
results = parser.search('blur')
```

详见：[文件列表工具指南](./tools/FILELIST_GUIDE.md)

## 相关文档

- [多产品架构](./__docs/多产品架构.md)
- [开发服务器文档](./DEV_SERVER.md)
- [文件列表工具指南](./tools/FILELIST_GUIDE.md)
- [路径配置说明](./PATH_CONFIGURATION.md)
- [客户端集成示例](./CLIENT_INTEGRATION_EXAMPLE.md)